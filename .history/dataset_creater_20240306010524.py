...
def generate_sales_data(length, trend_type, seasonality_coef, noise_std):
    ...
    # Generate sales data without negative values
    sales_data = np.maximum(trend + seasonality + noise, 0)
...
df_sales = pd.DataFrame(index=pd.date_range(start='1/1/2010', periods=months, freq='M'))
